...
function format_typo_post($post_ID)
{
	...
	//Get the array of strings made of text without the insides of the <pre> tag
	$without_pre = preg_split('@<pre[^>]*?>.+</pre>@s', $content);
    //Get the array of <pre> tags from the text
	$n_pre = preg_match_all('@<pre[^>]*?>.+</pre>@s', $content, $pre);

	for ($k = 0; $k <= $n_pre; $k++)
	{
		//Get the array of strings made of text without HTML tags
		$without_html = preg_split('@<[\/\!]*?[^<>]*?>@', $without_pre[$k]);
	    //Get the array of HTML tags from the text
		$amount = preg_match_all('@<[\/\!]*?[^<>]*?>@', $without_pre[$k], $html);

		//Concatenate the text, making necessary replacements
		for ($i = 0; $i <= $amount; $i++)
		{
		   	//Replace double (and more) spaces with single spaces
			$without_html[$i] = preg_replace('@ {2,}@', ' ', $without_html[$i]);

			//Add m-dashes with a preceeding non-breaking space, in place of the hyphens, where neeeded
			$without_html[$i] = str_replace(' - ', '&nbsp;&#8212; ', $without_html[$i]);
		...
		if ($k < $n_pre) $new_content .= $pre[0][$k];
	}
